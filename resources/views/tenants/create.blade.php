...
<div class="max-w-md mx-auto space-y-6">
    ...
    <div class="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
    ...
    <div class="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl space-y-5">
        ...
        <form action="{{ route('tenants.join') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="invite_code" required placeholder="{{ __('app.invite_code_placeholder') }}"
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-900 font-mono font-bold text-sm focus:border-amber-500 focus:bg-white focus:ring-1 focus:ring-amber-500">
                @error('invite_code') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <button type="submit" class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition shadow-sm active:scale-[0.98]">
                {{ __('app.join_btn') }}
            </button>
        </form>
    </div>
