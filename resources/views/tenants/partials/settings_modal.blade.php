...
<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm"
    ...
    <div @click.outside="document.getElementById('settings-modal-wrapper')?.remove()"
         class="w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-white border border-slate-200 rounded-t-lg sm:rounded-lg p-6 shadow-2xl space-y-5">
        
        <!-- Header -->
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            <div class="flex items-center gap-2.5">
                <div class="w-10 h-10 rounded-md bg-amber-50 border border-amber-200 flex items-center justify-center text-xl">
                    ⚙️
                </div>
                <div>
            ...
            <button type="button" @click="document.getElementById('settings-modal-wrapper')?.remove()" class="p-1.5 text-slate-400 hover:text-slate-600 rounded-md hover:bg-slate-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>

        <!-- Segmented Navigation Tabs -->
        <div class="flex p-1 bg-slate-100 rounded-md border border-slate-200">
            <button type="button"
                    @click="activeTab = 'categories'"
                    class="flex-1 py-2 rounded-md text-xs font-bold transition flex items-center justify-center gap-1.5"
                ...
                <span class="px-1.5 py-0.5 rounded text-[10px] bg-amber-100 text-amber-900 font-extrabold">{{ $categories->count() }}</span>
            </button>
            <button type="button"
        ...
        <div x-show="activeTab === 'categories'" class="space-y-6">
            ...
            <div class="space-y-2.5">
                ...
                <div class="divide-y divide-slate-100 border border-slate-200 rounded-md overflow-hidden bg-slate-50/50">
                    @forelse($categories as $cat)
                        <div class="p-3 sm:p-3.5 bg-white flex items-center justify-between gap-3 hover:bg-slate-50/80 transition">
                            <div class="flex items-center gap-3 min-w-0">
                                <span class="w-8 h-8 rounded-md bg-slate-100 border border-slate-200 flex items-center justify-center text-base flex-shrink-0">
                                    {{ $cat->emoji }}
                                </span>
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        <span class="text-xs font-bold text-slate-900 truncate">{{ $cat->name }}</span>
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border {{ $cat->badge_class }}">
                                            {{ $cat->color }}
                                        </span>
                                    </div>
                            ...
                            <div class="flex items-center gap-1.5 flex-shrink-0">
                                ...
                                <button hx-get="{{ route('categories.edit', $cat->id) }}"
                                        ...
                                        class="px-2.5 py-1.5 rounded-md bg-slate-100 hover:bg-amber-100 hover:text-amber-900 text-slate-700 text-xs font-semibold transition flex items-center gap-1">
                                    <span>✏️</span>
            ...
            <div class="p-4 rounded-md bg-amber-50/50 border border-amber-200/80 space-y-3">
                <div class="flex items-center gap-2">
                ...
                <form action="{{ route('categories.store') }}" method="POST"
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="{{ __('app.category_name_placeholder') }}"
                               class="w-full px-3.5 py-2 bg-white border border-slate-300 focus:border-amber-500 rounded-md text-slate-900 text-xs font-semibold">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-1.5 p-2 bg-white border border-slate-200 rounded-md max-h-24 overflow-y-auto">
                            <template x-for="em in ['💼', '⚡', '🚀', '👥', '📋', '🎯', '🎨', '📊', '🛠️', '💡', '💰', '🔒', '📦', '🏷️', '📢', '💻', '🍕', '✨', '📝', '🛒', '🔧', '📈', '🤝', '⚙️', '📂', '🧪', '📱', '🏆', '🔔']">
                                <button type="button" @click="newEmoji = em"
                                        class="w-7 h-7 rounded flex items-center justify-center transition text-sm"
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 p-2 bg-white border border-slate-200 rounded-md">
                            <template x-for="c in [
                                ...
                                <button type="button" @click="newColor = c.id"
                                        class="w-6 h-6 rounded transition flex items-center justify-center ring-offset-2"
                    ...
                    <div class="pt-1">
                        <button type="submit" class="w-full py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-md shadow-sm active:scale-95 transition">
        ...
        <div x-show="activeTab === 'workspace'" class="space-y-6">
            ...
            <form action="{{ route('tenants.update', $tenant->id) }}" method="POST"
                  ...
                  hx-target="body"
                  @htmx:after-request="document.getElementById('settings-modal-wrapper')?.remove()"
                  class="p-4 rounded-md bg-slate-50 border border-slate-200 space-y-3">
                @csrf
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="name" value="{{ $tenant->name }}" required
                               class="flex-1 px-3.5 py-2.5 bg-white border border-slate-300 focus:border-amber-500 rounded-md text-slate-900 text-xs font-bold">
                        <button type="submit" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-md shadow-sm active:scale-95 transition">
                            {{ __('app.save') }} 💾
                        </button>
                    </div>
                </div>
            ...
            <!-- Team Invite Code -->
            <div class="p-4 rounded-md bg-slate-50 border border-slate-200 space-y-2.5"
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <input type="text" readonly value="{{ $tenant->invite_code }}"
                           class="flex-1 px-3.5 py-2 bg-white border border-slate-300 rounded-md text-xs font-mono font-bold text-slate-800 select-all">
                    <div class="flex items-center gap-1.5">
                        <button type="button"
                                @click="navigator.clipboard.writeText('{{ $tenant->invite_code }}'); copiedCode = true; setTimeout(() => copiedCode = false, 2000)"
                                class="flex-1 sm:flex-initial px-3.5 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 text-xs font-bold rounded-md transition flex items-center justify-center gap-1.5">
                        ...
                        <button type="button"
                                ...
                                class="flex-1 sm:flex-initial px-3.5 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-md transition flex items-center justify-center gap-1.5 shadow-sm">
