...
<div class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm"
    ...
    <div @click.outside="document.getElementById('category-edit-modal-wrapper')?.remove()"
         class="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl space-y-4">
        ...
        <div class="flex items-center justify-between pb-2 border-b border-slate-100">
            ...
            <button type="button" @click="document.getElementById('category-edit-modal-wrapper')?.remove()" class="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100">
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $category->name }}" required
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-slate-900 text-sm focus:border-amber-500 font-semibold">
            </div>
            ...
            <div class="flex gap-2 justify-end pt-2 border-t border-slate-100">
                ...
                <button type="submit" class="px-5 py-2 text-xs font-bold bg-amber-500 text-slate-950 rounded-lg hover:bg-amber-400 shadow-sm active:scale-95 transition">
                    {{ __('app.save_changes') }} 💾
                </button>
            </div>
        </form>
