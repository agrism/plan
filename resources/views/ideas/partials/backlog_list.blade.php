...
@else
    @foreach($backlogIdeas as $idea)
        ...
        <div class="group relative rounded-md bg-white border border-slate-200/90 hover:border-slate-300 transition p-4 shadow-sm hover:shadow-md flex flex-col gap-2.5"
             x-data="{ expanded: false }">
            
            <!-- Card Header: Category, Author Badge, Edit & Delete Buttons -->
            <div class="flex items-center justify-between gap-2">
                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-xs font-bold border {{ $idea->category_badge_class }}">
            ...
            <div @click="{{ $hasDetails ? 'expanded = !expanded' : '' }}"
                ...
                @if($hasDetails)
                    <button type="button" @click.stop="expanded = !expanded"
                            ...
                            class="p-1 text-slate-400 hover:text-slate-600 rounded hover:bg-slate-100 transition flex-shrink-0 mt-0.5">
            ...
            @if($hasDetails)
                <div x-show="expanded" x-collapse x-cloak class="space-y-3 pt-2 border-t border-slate-100/90" @click.stop>
                    ...
                    <!-- Optional Image -->
                    @if($idea->image_url)
                        <div class="w-full h-40 rounded-md overflow-hidden bg-slate-100 border border-slate-100">
                    ...
                    @if($idea->description)
                        <div class="bg-slate-50 p-3 rounded-md border border-slate-100 text-xs text-slate-700 leading-relaxed whitespace-pre-line">
                            {{ $idea->description }}
                        </div>
                    @endif
                            ...
                            @foreach($idea->processed_links as $linkItem)
                                ...
                                <div class="flex items-center gap-1.5 text-xs">
                                    <a href="{{ $linkItem['url'] }}" target="_blank" rel="noopener noreferrer"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 font-medium truncate max-w-full transition">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-slate-100 mt-0.5" @click.stop>
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <button hx-post="{{ route('ideas.react', $idea->id) }}"
                            hx-target="body"
                            type="button"
                            class="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold transition {{ $idea->reactions->where('user_id', auth()->id())->isNotEmpty() ? 'bg-amber-100 text-amber-900 border border-amber-300 shadow-sm' : 'bg-slate-100 text-slate-600 hover:text-slate-900 hover:bg-slate-200' }}">
                        <span>👍</span>
                    ...
                    <button @click.stop="expanded = !expanded"
                            ...
                            class="flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold transition {{ $idea->comments->isNotEmpty() ? 'bg-slate-100 text-slate-800 border border-slate-200 shadow-sm' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100' }}">
                        <span>💬</span>
                ...
                <button @click.stop="window.openScheduleModal({{ $idea->id }}, '{{ addslashes($idea->title) }}', '{{ $idea->scheduled_date ? $idea->scheduled_date->toDateString() : '' }}', '{{ route('ideas.schedule', $idea->id) }}', false)"
                        type="button"
                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-800 text-xs font-bold transition">
                    <span>📅</span>
