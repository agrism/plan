<div id="task-comments-container-{{ $task->id }}" class="space-y-3 pt-2">
    ...
    <div class="flex items-center justify-between">
        <h4 class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
            ...
            <span class="px-1.5 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-[10px] font-bold text-slate-600">
                {{ $task->comments->count() }}
            </span>
        </h4>
    </div>
    ...
    <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
        @if($task->comments->isEmpty())
            <div class="p-3 text-center bg-slate-50 border border-dashed border-slate-200 rounded-xl text-slate-400 text-xs">
                <span class="text-base block mb-0.5">💭</span>
                <p>{{ __('app.no_comments') }}</p>
            </div>
        @else
            @foreach($task->comments as $comment)
                <div class="p-2.5 rounded-xl bg-slate-50 border border-slate-200/90 text-xs space-y-1.5 hover:bg-white transition group">
    ...
    <form hx-post="{{ route('ideas.comments.store', $task->id) }}"
        ...
        <div class="flex-1">
            <textarea name="content"
                      ...
                      class="w-full px-3 py-2 bg-slate-50 border border-slate-300 focus:border-amber-500 focus:bg-white rounded-xl text-slate-900 text-xs resize-none placeholder:text-slate-400"></textarea>
        </div>
        <button type="submit"
                class="px-3 py-2 bg-amber-500 hover:bg-amber-400 active:scale-95 text-slate-950 font-bold text-xs rounded-xl shadow-sm transition flex-shrink-0 flex items-center gap-1">
