...
<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm"
    ...
    <div @click.outside="document.getElementById('edit-modal-wrapper')?.remove()"
         class="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white border border-slate-200 rounded-t-lg sm:rounded-lg p-6 shadow-2xl space-y-5">
        ...
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <button type="button" @click="document.getElementById('edit-modal-wrapper')?.remove()" class="p-1.5 text-slate-400 hover:text-slate-600 rounded-md hover:bg-slate-100">
        ...
        <form action="{{ route('ideas.update', $task->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ $task->title }}" required
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-amber-500 focus:bg-white rounded-md text-slate-900 font-semibold text-sm">
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="{{ __('app.description_placeholder') }}"
                          class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-amber-500 focus:bg-white rounded-md text-slate-900 text-xs resize-none">{{ $task->description }}</textarea>
            </div>
                ...
                <div class="space-y-2">
                    <template x-for="(link, index) in editLinks" :key="index">
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" @click="editLinks.splice(index, 1)" class="p-1.5 text-slate-400 hover:text-rose-600 rounded hover:bg-rose-50 text-xs" title="{{ __('app.delete') }}">✕</button>
                        </div>
                    </template>
                </div>
            ...
            <div>
                ...
                <input type="date" name="scheduled_date" value="{{ $task->scheduled_date ? $task->scheduled_date->toDateString() : '' }}"
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 focus:border-amber-500 focus:bg-white rounded-md text-slate-900 text-xs">
            ...
            <div>
                ...
                @if($task->image_url)
                    <div x-show="!removeImage" class="relative rounded-md overflow-hidden bg-slate-100 border border-slate-200 mb-2 p-2 flex items-center justify-between gap-3">
                        <img src="{{ $task->image_url }}" alt="{{ __('app.image') }}" class="w-16 h-12 object-cover rounded">
                        <div class="flex-1 text-xs text-slate-500 truncate">{{ __('app.existing_image') }}</div>
                        <button type="button" @click="removeImage = true" class="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded transition">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <template x-if="newImagePreview">
                        <div class="relative w-12 h-10 rounded overflow-hidden border border-slate-200 shadow-sm flex-shrink-0">
            ...
            <div class="flex items-center justify-between pt-3 border-t border-slate-100">
                <button type="button"
                        ...
                        class="px-3 py-2 text-rose-600 hover:bg-rose-50 rounded-md text-xs font-bold transition flex items-center gap-1.5">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 text-slate-950 font-bold text-xs rounded-md shadow-md shadow-amber-500/20 active:scale-95 transition">
                        {{ __('app.save_changes') }} 💾
                    </button>
                </div>
            </div>
