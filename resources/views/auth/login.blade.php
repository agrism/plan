...
<body class="min-h-full bg-slate-50 flex items-center justify-center p-4 relative">
    ...
    <div class="absolute top-4 right-4 flex items-center bg-white/80 backdrop-blur-sm border border-slate-200 rounded-md p-0.5 shadow-sm">
        <a href="{{ route('locale.switch', 'lv') }}"
           class="px-2.5 py-1 rounded text-xs font-bold transition flex items-center gap-1 {{ app()->getLocale() === 'lv' ? 'bg-amber-500 text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-900' }}"
           title="Latviešu">
            <span>🇱🇻</span>
            <span>LV</span>
        </a>
        <a href="{{ route('locale.switch', 'en') }}"
           class="px-2.5 py-1 rounded text-xs font-bold transition flex items-center gap-1 {{ app()->getLocale() === 'en' ? 'bg-amber-500 text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-900' }}"
           title="English">
            <span>🇬🇧</span>
            <span>EN</span>
        </a>
    ...
    <div class="w-full max-w-md bg-white border border-slate-200/90 rounded-lg p-6 sm:p-8 shadow-xl space-y-6">
                ...
                <div class="grid grid-cols-2 gap-2">
                    @foreach($demoUsers as $du)
                        <a href="{{ route('login.quick', $du->id) }}"
                           class="flex items-center gap-2 p-2.5 rounded-md bg-white border border-slate-200 hover:border-amber-400 hover:bg-amber-50/50 text-xs font-bold text-slate-800 shadow-sm transition">
