...
<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm"
    ...
    <div @click.outside="document.getElementById('profile-modal-wrapper')?.remove()"
         class="w-full max-w-lg max-h-[92vh] overflow-y-auto bg-white border border-slate-200 rounded-t-lg sm:rounded-lg p-6 shadow-2xl space-y-5">
        ...
        <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <button type="button" @click="document.getElementById('profile-modal-wrapper')?.remove()" class="p-1.5 text-slate-400 hover:text-slate-600 rounded-md hover:bg-slate-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>

        @if (session('status'))
            <div class="p-3.5 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold rounded-md flex items-center gap-2">
                <span>✓</span>
        ...
        @if ($errors->any())
            <div class="p-3.5 bg-rose-50 border border-rose-200 text-rose-800 text-xs font-bold rounded-md space-y-1">
        ...
        <form action="{{ route('profile.update') }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $user->name) }}" required
                       ...
                       class="w-full px-3.5 py-2.5 bg-white border border-slate-300 focus:border-amber-500 rounded-md text-slate-900 text-xs font-bold">
            </div>
            ...
            <div>
                ...
                @if(!$user->hasVerifiedEmail())
                    <div class="mt-2 p-2.5 bg-amber-50/80 border border-amber-200/80 rounded-md flex items-center justify-between gap-2">
                        ...
                        <button type="button"
                                ...
                                hx-swap="innerHTML"
                                class="px-2.5 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-[11px] rounded shadow-sm transition active:scale-95 whitespace-nowrap">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2 text-base p-2 bg-slate-50 border border-slate-200 rounded-md max-h-28 overflow-y-auto">
            ...
            <div class="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                <button type="button" @click="document.getElementById('profile-modal-wrapper')?.remove()"
                        class="px-4 py-2.5 text-xs font-bold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-md transition">
                    {{ __('app.cancel') }}
                </button>
                <button type="submit"
                        class="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-md shadow-sm active:scale-95 transition">
                    {{ __('app.save') }} 💾
                </button>
            </div>
        </form>
